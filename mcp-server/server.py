...
class EnhancedPhysioNetExtractor:
    ...
    async def extract_metadata(self, url: str) -> dict:
        ...
        try:
            ...
            metadata["Data_Collection_Period"] = characteristics["data_collection_period"]
            
            # Extract file structure
            file_info = self.extract_file_structure(soup)
            metadata["Total_Size_Compressed"] = file_info["total_size_compressed"]
            metadata["Total_Size_Uncompressed"] = file_info["total_size_uncompressed"]
            metadata["Main_Folders"] = file_info["main_folders"]
            
            # Extract ethics and funding
            ethics_funding = self.extract_ethics_and_funding(soup, page_text)
            metadata["Ethics_Approval"] = ethics_funding["ethics_approval"]
            metadata["IRB_Number"] = ethics_funding["irb_number"]
            metadata["Funding_Sources"] = ethics_funding["funding_sources"]
            
            # Extract citations
            citations = self.extract_citations_and_references(soup)
            metadata["Primary_Citation"] = citations["primary_citation"]
            metadata["Related_Publications"] = citations["related_publications"]
            
            # Extract research applications
            metadata["Research_Applications"] = self.extract_research_applications(page_text, soup)
            
            # Extract limitations
            metadata["Limitations"] = self.extract_limitations_and_challenges(soup, page_text)
            
            # Extract access requirements
            access_info = self.extract_access_requirements(soup, page_text)
            metadata["Access_Type"] = access_info["access_type"]
            metadata["Licensing_or_Availability"] = access_info["license"]
            metadata["Training_Required"] = access_info["training_required"]
            metadata["DUA_Required"] = access_info["dua_required"]
            metadata["Credentialing_Required"] = access_info["credentialing_required"]
            
            # Calculate metadata completeness
            filled_fields = sum(1 for k, v in metadata.items() 
                              if k not in ["id", "curated_date", "Dataset_URL"] and 
                              v != "Not specified" and v != [] and v is not False)
            total_fields = len([k for k in metadata.keys() 
                              if k not in ["id", "curated_date", "Dataset_URL"]])
            ...
            keywords = []
            if metadata["Title"] != "Not specified":
                title_words = re.findall(r'\b[A-Z][a-z]+\b', metadata["Title"])
                keywords.extend(title_words[:5])
            if metadata["Description"] != "Not specified":
                desc_words = re.findall(r'\b\w{4,}\b', metadata["Description"].lower())
                keywords.extend(desc_words[:10])
            metadata["Keywords_Used"] = list(set(keywords))[:15]
...
extractor = EnhancedPhysioNetExtractor()

@app.list_tools()
async def list_tools() -> list[Tool]:
    """List all available MCP tools for PhysioNet dataset curation"""
    return [
        Tool(
            name="search_physionet",
            description="Search for PhysioNet datasets by query string or URL. Returns up to 5 matching dataset results with titles and URLs.",
            inputSchema={
                "type": "object",
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "Search query string or direct PhysioNet dataset URL"
                    }
                },
                "required": ["query"]
            }
        ...
        Tool(
            ...
            description="Extract comprehensive metadata from a PhysioNet dataset URL. Returns detailed information including modalities, clinical conditions, dataset characteristics, authors, citations, and more.",
            inputSchema={
                "type": "object",
                "properties": {
                    "url": {
                        "type": "string",
                        "description": "Full URL to the PhysioNet dataset page"
                    }
                },
                "required": ["url"]
            }
        ...
        Tool(
            ...
            description="Fetch, analyze, and save a dataset to the curated database. This tool extracts comprehensive metadata and saves it to the database, updating if the dataset already exists.",
            inputSchema={
                "type": "object",
                "properties": {
                    "url": {
                        "type": "string",
                        "description": "Full URL to the PhysioNet dataset page to curate"
                    }
                },
                "required": ["url"]
            }
        ...
        Tool(
            ...
            description="Curate multiple datasets in batch. Processes a list of URLs sequentially with delays between requests to avoid rate limiting.",
            inputSchema={
                "type": "object",
                "properties": {
                    "urls": {
                        "type": "array",
                        "items": {"type": "string"},
                        "description": "Array of PhysioNet dataset URLs to curate"
                    }
                },
                "required": ["urls"]
            }
        ...
        Tool(
            ...
            description="Get statistics and information about the curated datasets database, including total count and recent additions.",
            inputSchema={
                "type": "object",
                "properties": {}
            }
        ...
        Tool(
            name="get_dataset_by_url",
            description="Retrieve a specific curated dataset from the database by its URL.",
            inputSchema={
                "type": "object",
                "properties": {
                    "url": {
                        "type": "string",
                        "description": "URL of the dataset to retrieve from database"
                    }
                },
                "required": ["url"]
            }
        ...
        Tool(
            name="search_curated_datasets",
            description="Search through curated datasets in the database by keyword, modality, condition, or other metadata fields.",
            inputSchema={
                "type": "object",
                "properties": {
                    "query": {
                    ...
                    "field": {
                        "type": "string",
                        "description": "Optional: specific field to search (e.g., 'Title', 'Modalities_List', 'Clinical_Condition')",
                        "enum": ["Title", "Modalities_List", "Clinical_Condition", "Authors", "Year", "Description"]
                    }
                },
                "required": ["query"]
            }
...
async def call_tool(name: str, arguments: Any) -> list[TextContent]:
    ...
    try:
        if name == "search_physionet":
            ...
            if not url:
                return [TextContent(type="text", text=json.dumps({"error": "URL parameter is required"}, indent=2))]
            ...
            print(f"Curating dataset: {url}", file=sys.stderr)
            ...
            if not urls or not isinstance(urls, list):
                return [TextContent(type="text", text=json.dumps({"error": "URLs array is required"}, indent=2))]
            ...
            summary = {
                "total": len(urls),
                "successful": len([r for r in results if r.get("status") in ["saved", "updated"]]),
            ...
            for dataset in datasets:
                # Count modalities
                if isinstance(dataset.get("Modalities_List"), list):
                    for mod in dataset.get("Modalities_List", []):
                        modalities_count[mod] = modalities_count.get(mod, 0) + 1
                
                # Count conditions
                if isinstance(dataset.get("Conditions_List"), list):
                    for cond in dataset.get("Conditions_List", []):
            ...
            stats = {
                ...
                "top_modalities": dict(sorted(modalities_count.items(), key=lambda x: x[1], reverse=True)[:10]),
                "top_conditions": dict(sorted(conditions_count.items(), key=lambda x: x[1], reverse=True)[:10]),
                "years_distribution": dict(sorted(years_count.items(), reverse=True)),
                "recent_datasets": [
                    {
                        "title": d.get("Title", "Unknown"),
                        "year": d.get("Year", "Unknown"),
                        "curated_date": d.get("curated_date", "Unknown"),
                        "url": d.get("Dataset_URL", "")
                    }
        ...
        elif name == "get_dataset_by_url":
            url = arguments.get("url", "")
            if not url:
                return [TextContent(type="text", text=json.dumps({"error": "URL parameter is required"}, indent=2))]
            
            datasets = extractor.load_database()
            matching = [d for d in datasets if d.get("Dataset_URL") == url]
            
            if matching:
                return [TextContent(type="text", text=json.dumps(matching[0], indent=2, ensure_ascii=False))]
            else:
                return [TextContent(type="text", text=json.dumps({"error": "Dataset not found in database"}, indent=2))]
        
        elif name == "search_curated_datasets":
            query = arguments.get("query", "").lower()
            field = arguments.get("field", None)
            ...
            for dataset in datasets:
                ...
                if field:
                    ...
                    field_value = dataset.get(field, "")
                    if isinstance(field_value, list):
                        match = any(query in str(item).lower() for item in field_value)
                    else:
                        match = query in str(field_value).lower()
                else:
                    # Search across all text fields
                    searchable_fields = [
                        "Title", "Description", "Abstract_Full", "Clinical_Condition",
                        "Physiological_Modality", "Authors", "Keywords_Used"
                    ]
                    for search_field in searchable_fields:
                        field_value = dataset.get(search_field, "")
                        if isinstance(field_value, list):
                            if any(query in str(item).lower() for item in field_value):
                                match = True
                                break
                        elif query in str(field_value).lower():
                            match = True
                            break
                ...
                if match:
                    results.append({
                        ...
                        "year": dataset.get("Year", "Unknown"),
                        "modalities": dataset.get("Modalities_List", []),
                        "conditions": dataset.get("Conditions_List", []),
                        "curated_date": dataset.get("curated_date", "Unknown")
                    })
...
async def main():
    ...
    print(f"PhysioNet Dataset Curator MCP Server", file=sys.stderr)
    print(f"Database file: {DB_FILE}", file=sys.stderr)
    print(f"Database exists: {DB_FILE.exists()}", file=sys.stderr)
    
    if DB_FILE.exists():
        datasets = extractor.load_database()
        print(f"Loaded {len(datasets)} existing datasets", file=sys.stderr)
